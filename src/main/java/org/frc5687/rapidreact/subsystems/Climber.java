...
/**
 * The Climber subsytem consists of two hooked arms on spring-loaded cylinders, restrained by
 * winches. Paying the winch out allows the hook to extend, retracting the winch pulls the hook
 * down. One arm is stationary, the other rocks foreward and back driven by a pneumatic piston.
 *
 * <p>The winches are driven by baby neos on sparkmax controllers (with integral encoders) and the
 * each arm has a pair of hall-effect sensors to indicate when they are UP or DOWN.
 *
 * <p>The winches are controlled by ProfiledPIDControllers, with goals set using setStaGoal and
 * setRockGoal. The controllers are enabled/disabled using the enable
 */
public class Climber extends OutliersSubsystem {
    ...
    public Climber(OutliersContainer container, DriveTrain driveTrain) {
        ...
        //        logMetrics("Stationary/Position", "Stationary/Goal", "Stationary/Enabled",
        // "Stationary/Speed", "Stationary/Up", "Stationary/Up", "Rocker/Position", "Rocker/Goal",
        // "Rocker/Enabled", "Rocker/Speed", "Rocker/Up", "Rocker/Down", "Rocker Cylinder");
        ...
        _stationaryArmWinch.configSupplyCurrentLimit(
                new SupplyCurrentLimitConfiguration(
                        true,
                        Constants.Climber.ARM_CURRENT_LIMIT,
                        Constants.Climber.CURRENT_THRES,
                        Constants.Climber.ARM_THRES_TIME));
        ...
        //        _stationaryArmWinch.configReverseSoftLimitThreshold((int)
        // (Constants.Climber.STATIONARY_RETRACTED_METERS/Constants.Climber.TICKS_TO_METERS), 30);
        ...
        _rockerArmWinch.configSupplyCurrentLimit(
                new SupplyCurrentLimitConfiguration(
                        true,
                        Constants.Climber.ARM_CURRENT_LIMIT,
                        Constants.Climber.CURRENT_THRES,
        ...
        //        _rockerArmWinch.configReverseSoftLimitThreshold((int)
        // (Constants.Climber.ROCKER_RETRACTED_METERS/Constants.Climber.TICKS_TO_METERS), 30);
        ...
        _rocker =
                new DoubleSolenoid(
                        ...
                        RobotMap.PCH.CLIMBER_IN,
                        RobotMap.PCH.CLIMBER_OUT);
    ...
    /**
     * Directly sets the speed of the stationary arm winch. Note that this is intended to be called
     * only by the climber's controllers.
     *
     * @param speed
     */
    ...
    /**
     * Directly sets the speed of the rocker arm winch. Note that this is intended to be called only
     * by the climber's controllers.
     *
     * @param speed
     */
    ...
    /**
     * Stops the stationary arm by disabling the PID controller, setting the winch speed to 0, and
     * setting brake mode.
     */
    ...
    /**
     * Stops the rocker arm by disabling the PID controller, setting the winch speed to 0, and
     * setting brake mode.
     */
    ...
    public void setStaGoalMeters(double goalMeters) {
        ...
        _stationaryArmWinch.set(
                ControlMode.MotionMagic, (_staGoal / Constants.Climber.TICKS_TO_METERS));
    ...
    public void setRockGoalMeters(double goalMeters) {
        ...
        _rockerArmWinch.set(
                ControlMode.MotionMagic, (_rockGoal / Constants.Climber.TICKS_TO_METERS));
     ...
     * Using the rocker arm and the stationary arms encoders with the string to get the distance the
     * arm is extened, that is used as the input to the PID controllers
     */
    ...
    /**
     * Checks to see if the stationary arm motor is stalled. It is stalled if the amperage draw is >
     * Constant and it has not moved in more than Constant cycles
     */
    ...
    /**
     * Checks to see if the rocker arm motor is stalled. It is stalled if the amperage draw is >
     * STALL_CURRENT and it has not moved in more than MAX_STALL_CYCLES cycles
     */
    public boolean isRockArmStalled() {
        ...
        info(
                "Rocker current "
                        + _rockerArmWinch.getSupplyCurrent()
                        + "  rocker velocity "
                        + _rockerArmWinch.getSelectedSensorVelocity()
                        + " rocker cycles "
                        + _rockStallCycles);
    ...
    /**
     * Checks to see if an arm is stalling. An arm is stalling if the controller is sending more
     * than STALL_CURRENT current but the encoder is reading less that STALL_MIN_RPM rotations.
     */
    private boolean isArmStalled(TalonFX controller) {
        return controller.getSupplyCurrent() > Constants.Climber.STALL_CURRENT
                && Math.abs(controller.getSelectedSensorVelocity())
                        < Constants.Climber.STALL_MIN_RPM;
    }
